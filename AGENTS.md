...
| Repos | **Wrapper superproject with 3 git submodules**: `api`, `frontend`, `backoffice`. This repo IS the wrapper (holds `docs/`, `openspec/` SDD, `AGENTS.md`, docker-compose, submodule pointers). Astro demo lives in `legacy-demo/` (reference, removed once ported). |
