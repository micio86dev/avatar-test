...
scope here — and it remains out of scope for logout even though the
user-scoped revocation primitive now exists (`RefreshTokenStore::revokeAllForUser`,
added for out-of-session password reset). Logout MUST NOT start calling it:
"sign out everywhere" is a deliberate user action, not the meaning of
signing out of one device.
