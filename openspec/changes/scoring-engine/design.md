...
### D7 — Real LLM binding + versioning ⚠ D25 GAP (STOP-and-report)
Production `LLMProvider` binding is the Anthropic (Claude) path; `AI_TEST_MODEL=claude-haiku-4-5-20251001` for the `@ai` group (versioned ID from D36 — NOT the alias `claude-haiku-4-5`; determinism at `temperature=0` requires the pinned version, not an alias that may resolve to a different model revision). `model_version`/`prompt_version` are config-pinned and recorded on `evaluations` + `ai_requests`. **BLOCKER**: the D25 Version Catalog (`project-skeleton-ci/design.md`) lists **no Anthropic/Claude PHP SDK** (only HeyGen/Daily/MediaPipe JS SDKs). Per the Dependency Resolution Policy, the apply phase MUST **STOP and report** the exact package+version to a human and add it to D25 before installing — never pick/pin an SDK silently. Interim: the deterministic core + FakeLLMProvider need no SDK; only the real binding is blocked.
