...
- [ ] B2a.1 Migration `*_add_unassessable_reason_to_indicator_scores.php`: nullable `unassessable_reason` column (0.4 naming — product owner override); backfill `UPDATE indicator_scores SET unassessable_reason = 'model_declared' WHERE score = -1` (every existing `-1` row predates per-indicator isolation, so this is a statement of fact); equivalence CHECK `(score = -1) = (unassessable_reason IS NOT NULL)`. `down()` drops CHECK then column — no data precondition.
...
- [ ] B2a.4 **RED** `api/tests/Unit/DTOs/Scoring/IndicatorScoreDTOTest.php`: `asUnassessable(IndicatorFailureReason)` returns a new readonly instance — `score: -1`, `excerpts: []`, `explanation` preserved, `unassessableReason` set.
- [ ] B2a.5 **GREEN** `api/app/DTOs/Scoring/IndicatorScoreDTO.php` — add `?IndicatorFailureReason $unassessableReason`, `asUnassessable()`.
...
- [ ] B2a.8 `api/app/Models/IndicatorScore.php` — add `unassessable_reason` attribute; fix stale docblock (also fix `2026_07_22_000003_create_indicator_scores_table.php` comment).
...
- [ ] B3.1 **RED** `api/tests/Feature/Admin/EvaluationSerializerTest.php`: a `behaviors[]` entry with `score = -1, unassessable_reason = 'excerpt_unverifiable'` serializes `unassessable_reason: "excerpt_unverifiable"`; a legally-scored entry serializes `unassessable_reason: null` (0.4 naming — not `reason`, not `failure_reason`).
- [ ] B3.2 **GREEN** `AdminEvaluationSerializer` — add `unassessable_reason` to each `behaviors[]` entry; update `EvaluationKeySetTest` (A4.3) expected list.
- [ ] B3.3 **RED+GREEN** `api/tests/Unit/C10/EvaluationPayloadAssemblerTest.php`: `behaviors[]` gains additive `unassessable_reason` in the `evaluation` webhook payload; `payload_version` unchanged (D10).
- [ ] B3.4 Add `report.indicatorUnassessableReason.{model_declared,excerpt_unverifiable,score_illegal,unknown}` to `backoffice` `en.json`/`it.json` (author `it` in this PR, D11).
- [ ] B3.5 **RED+GREEN** `backoffice/tests/unit/components/atoms/ScoreChip.spec.ts`: the `unassessable` chip's screen-reader label/`title` uses the `indicatorUnassessableReason` key when `unassessable_reason` is present, replacing the generic `report.chip.unassessable`; visual density unchanged.
