...
- **Submodule `api`** — **Laravel 13** + Eloquent + **PHP 8.5** + **PostgreSQL 17** (`pgvector/pgvector:0.8.0-pg17` local/CI; **Supabase** stage/prod) + **Redis 8** / Horizon, **API-only** (no Blade UI). **Scramble** (`dedoc/scramble ^0.12`) installed to publish the OpenAPI spec (`openapi.json`). Pest `^3.0` + PCOV coverage.
...
- **Autonomous implementation guardrails:** a **Dependency Resolution Policy** (STOP-and-report on any unresolvable pinned dependency — never downgrade, replace, unpin, or substitute; see design D37), a **documented required local toolchain** in `docs/dev-setup.md` (see design D38), **Bun as the sole Nuxt package manager** (no pnpm/npm/yarn in the new apps), and **machine-facing API responses left un-localized** (e.g. `/api/health`). Observability in C1 is **health endpoints only**; Sentry/Clarity/GA4/Pulse/Cloudflare/`ai_requests`/domain-events are deferred to their owning slices (see `openspec/specs/observability/spec.md`).
...
- `docker-compose.yml` in the wrapper provisions **PostgreSQL 17** (`pgvector/pgvector:0.8.0-pg17`, with an init script creating both `beai` and `beai_test` databases) + **Redis 8** (`redis:8.0-alpine`) + Mailpit (`axllent/mailpit:v1.22`) + the three app services (built from each app's multi-stage Dockerfile); each submodule's `.env.example` points at the compose service names. All base image tags pinned — see Version Catalog in design.md D25.
...
| Local infra drift (versions) | Low | All image tags pinned per Version Catalog (design.md D25); `pgvector/pgvector:0.8.0-pg17` must match the Supabase PostgreSQL major version (17); bumps are deliberate and reviewed |
...
- [ ] `docker-compose up` brings **PostgreSQL 17** (`pgvector/pgvector:0.8.0-pg17`) + **Redis 8** + Mailpit + the three app services (built from their Dockerfiles); all three apps boot against them; both `beai` and `beai_test` databases are provisioned on first start.
...
- [ ] Stack is consistent across `CLAUDE.md`, `openspec/config.yaml`, `ROADMAP.md`, and every C1 artifact: Laravel 13 + PHP 8.5 + PostgreSQL 17 (pgvector) + Redis 8 + Bun + Nuxt 4 + JWT; zero references to MySQL (except as a rejected alternative), Sanctum (except as "NOT Sanctum"), Laravel 12, `web/`, or pnpm/npm/yarn in the new apps.
- [ ] `docs/dev-setup.md` documents the required local toolchain (design D38) and the Dependency Resolution Policy (design D37); both are referenced from `CLAUDE.md`.
- [ ] Machine-facing API responses (e.g. `/api/health` → `{"status":"ok"}`) are returned literally and are NOT localized; no C1 observability integration beyond health endpoints exists.
