...
- [ ] 4.7 **BLOCKED on a signature, not on engineering**: real durations. Everything BEAI owns is done — `docs/retention-decision-brief.md` states what each of the four classes actually deletes and what survives, names the three purposes that set the bounds (delivery, the Article 22 right to contest — which is the binding LOWER bound and the one a privacy review reliably forgets, because the pressure always runs toward shorter — and defending a discrimination claim, which is per-jurisdiction and belongs to the customer), flags that proctoring snapshots are the highest-risk class and may attract Article 9, lists the four business facts BEAI must supply before anyone can sign, and gives the exact five environment variables that ratify it. Verified in production: `beai:purge-expired-data --dry-run` reports DISABLED and purges nothing. One design consequence surfaced there rather than discovered later: `config/retention.php` is GLOBAL, so if any customer contract requires a shorter window than the platform default the purge as built cannot honour it — ask the per-tenant question during ratification, while it is still cheap. Original framing:  Open decision #2 needs legal sign-off, and the sign-off must cover `webhook_deliveries.payload` and `participants.display_name`, which postdate the original framing. The mechanism is built so that ratification is a config change, not a code change.
