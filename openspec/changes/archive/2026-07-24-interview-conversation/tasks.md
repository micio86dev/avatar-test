...
- [x] 5.6 Update `buildSuccessResponse()` to accept `?string $promptVersion` and add `'prompt_version' => $promptVersion` inside `question_context`. **The composed `system_prompt` is server-side only and MUST NOT appear in the `/start` response** (delta spec `specs/interview-session/spec.md` L20-28; reconciled in commit 53b1e36). Pass `promptVersion` from the call sites in `handleIssuePending` and `handleResumeInCorso`. Make 5.1–5.4 green.
