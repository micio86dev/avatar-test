...
It MUST declare `->withoutMiddleware(TenantContext::class)` (see tenancy delta).
...
credential — they are consumed once at the exchange endpoint only. On the `api` guard,
