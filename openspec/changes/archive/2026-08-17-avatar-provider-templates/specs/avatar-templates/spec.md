# Delta: avatar-templates — provider template catalogue, single-active invariant, provider payload wiring

This is the base capability the promoted `openspec/specs/avatar-templates/spec.md`
says is missing: "the catalogue, one active template per organization, provider
immutability, field specs." Written from the shipped code and its test suite
(`api/tests/Feature/C14/*`, `backoffice/tests/unit/avatar-template-*`), not from
the original proposal's intent — where the two differ, the code is what a
candidate or an operator actually experiences.
...
Both are upfront scope decisions recorded in the original proposal
(`proposal.md`, "Explicitly out of scope"), not work that stalled mid-change —
every other unit in the six-PR plan shipped and merged.
