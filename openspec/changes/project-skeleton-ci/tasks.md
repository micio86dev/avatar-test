...
- [ ] 6.3 **[i18n mandate — api]** Audit `api/app/` for any inline user-facing string literals in controllers, responses, and form request messages. All user-facing strings MUST use `__('key')`. In C1 the health endpoint returns `{ "status": "ok" }` — either key this (`__('health.status')`) or document the exception. Add `lang/it/messages.php` and `lang/en/messages.php` entries for any keyed strings. Verify: `rg '"[A-Za-z ]+"' app/Http/` returns only non-user-facing technical values (class names, config keys, log context).
...
- [ ] 7.5 **[bun audit — Nuxt CI]** Add to both Nuxt CI workflows a required blocking step:
  ...
  - name: Security audit
    run: bun audit
  ```
  Exits non-zero on HIGH/CRITICAL. If `bun audit` is not yet stable, use `npx audit-ci --high` as a fallback (but prefer native `bun audit`).
