...
(`pgvector/pgvector:pg17-alpine`) + Redis 8 + Mailpit **plus the three app
...
## Data Flow
    ...
    wrapper (this repo)
      docker-compose ── pgvector/pgvector:pg17-alpine ─┐
                     ├─ redis:8.0-alpine ──────────────┤
                     └─ mailpit:v1.22 ────────────────┤
                                   ▼
...
| wrapper: `docker-compose.yml` | Create | PostgreSQL 17 (`pgvector/pgvector:pg17-alpine`) + Redis 8 (`redis:8.0-alpine`) + Mailpit (`axllent/mailpit:v1.22`) (pinned, named volumes, healthchecks) **+ 3 app services** (`api`, `frontend`, `backoffice`) built from each app's Dockerfile |
...
| `frontend`: `.husky/pre-commit` | Create | Husky pre-commit hook script: `npx lint-staged` |
...
| `api`: `.github/workflows/ci.yml` | Modify | Add `services.postgres` block (`pgvector/pgvector:pg17-alpine`, `POSTGRES_DB=beai_test`, healthcheck on port 5432 via `pg_isready`); add `php artisan migrate` step after `composer install`, before Pest |
...
- [x] Bun + Node version pins — **resolved in D25**: `oven/bun:1.3` + `node:24-slim`; exact patch locked in `bun.lockb`.
...
Authoritative reference for all pinned versions used in C1. Update this table when a version is intentionally bumped — no other place should diverge from it. Patch-level locking happens in `composer.lock` / `bun.lockb`; the entries below define the **minimum accepted minor version** and the **Docker image tag** used everywhere.

### Docker Base Images

| Image | Tag | Used in |
|-------|-----|---------|
| PostgreSQL + pgVector | `pgvector/pgvector:pg17-alpine` | docker-compose, CI services |
...
| PostgreSQL | `17` | Via `pgvector/pgvector:pg17-alpine`; must match Supabase project major |
...
| Bun | `1.3` | Via `oven/bun:1.3`; patch locked in `bun.lockb` |
| Node | `24 LTS` | Via `node:24-slim`; Long-Term Support active |
| Nuxt | `^4.0` | Latest stable 4.x; patch locked in `bun.lockb` |
