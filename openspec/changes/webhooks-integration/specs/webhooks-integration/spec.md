...
from ingress), a project reference, `status` ∈ `{completed, pending}`, a `version` field,
and `evaluation.text` per competency (score, `reliability`, behaviors). The
...
adding an `audio` key in a future version is additive, not breaking.
...
reference, a `version` field, and a per-competency progress list (`competency code` →
list of recorded responses with `question id` and `timestamp`). For a newly created
participant, ALL project competencies MUST be present in the list with empty response
lists. For an advancement trigger (competency-session end), the payload MUST reflect the
current cumulative response state across all competencies for that participant.
...
- THEN all 3 competency codes are present, each with an empty response list

#### Scenario: Advancement progress payload reflects cumulative state

- GIVEN a participant has completed competency INN (2 responses) and has not started the remaining competencies
- WHEN a `progress` payload is assembled after the INN competency-session ends
- THEN INN shows 2 responses and all other competencies show their current (possibly empty) response lists
